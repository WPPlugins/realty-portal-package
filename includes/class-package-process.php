			...
			if( $order['status'] != $order_status ) {
				...
				if( $order_status == 'completed' ) {
					...
					if( RP_Membership::is_membership() ) {
						...
						rp_mail($admin_email,
							sprintf( esc_html__( '[%s] New Payment received for Membership purchase','realty-portal-package' ), $site_name),
							$message);
						...
						rp_mail($user_email,
							sprintf( esc_html__( '[%s] Payment for membership successfully processed','realty-portal-package' ), $site_name),
							$message);
					} elseif( RP_Membership::is_submission() ) {
						$order['payment_type'] = esc_attr( get_post_meta( $order_id, '_payment_type', true ) );
						...
						rp_mail($user_email,
							sprintf( esc_html__( '[%s] Payment for Property listing successfully processed','realty-portal-package' ), $site_name),
							$message);
					}
				}
			} else {
